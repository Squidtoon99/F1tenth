...
def test_arbitrate_priority_safety_over_teleop_rl():
    ...
    assert cmd.brake_current_a == pytest.approx(10.0)
...
def test_arbitrate_rl_when_teleop_stale():
    ...
    assert cmd.drive_current_a == pytest.approx(8.0)
...
def test_validate_rl_rejects_out_of_range_and_stale_generation():
    cfg = _cfg()
    state = GateState(last_applied_generation=5)
    now = 1.0
    assert not validate_rl_command(
        _rl(drive_current_a=11.0), cfg, state.last_applied_generation, now
    )
    assert not validate_rl_command(
        _rl(generation=5), cfg, state.last_applied_generation, now
    )
    assert not validate_rl_command(
        _rl(received_s=0.5), cfg, state.last_applied_generation, now
    )
