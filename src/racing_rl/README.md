...
- `f1tenth_rl_vehicle/` — C++ `vehicle_obs` (track + odom → 392-dim observation),
...
- `f1tenth_rl_agent/sensor_racer` — recurrent no-localization sensor inference.
  Delta-steering artifacts are integrated by at most three degrees per 10 Hz
  decision and clamped to ±0.33 rad; incompatible absolute artifacts are rejected.
