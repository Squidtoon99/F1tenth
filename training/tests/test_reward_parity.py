...
mirror and asserts every reward term matches within a tight epsilon -- so the two
implementations cannot drift. It also pins a broad deterministic distribution
panel and checks that the global reward scale rescales totals without changing the
per-term structure.
"""
...
_TERMS = ("progress", "oob_penalty", "tyre_slip_penalty", "passing", "collision",
          "rear_end")
...
def test_kernel_matches_torch_mirror(rewards_mod):
    ...
    try:
        ...
        for step in range(16):
            ...
            # Parity is asserted only on rows that are on-track; the footprint
            # (kernel) vs center (mirror) off-course predicates diverge near the
            # boundary, so those rows are excluded from the term comparison.
            on_track = extras["metrics"]["oob_mask"] == 0
...
def test_distribution_regression_panel():
    ...
    try:
        ...
        # Progress-dominated (coefficient 1.0 * ds): per-step totals are small and
        # mostly positive. Broad bounds catch order-of-magnitude regressions only.
        assert -3.0 < float(q[0]) < 2.0
        assert -1.0 < float(q[1]) < 2.0
        assert 0.0 < float(q[2]) < 3.0
        assert float(q[3]) < 6.0
        assert -1.0 < float(flat.mean()) < 2.0
    finally:
        env.close()
