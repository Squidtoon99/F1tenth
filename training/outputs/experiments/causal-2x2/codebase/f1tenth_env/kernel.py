...
class RewardParams:
    ...
    terminal_oob_skip_seconds: wp.float32
    wall_contact_coefficient: wp.float32
    lee_wall_mode: wp.int32
...
def compute_reward_and_done(
    ...
    first_boundary_contact = off_track and env.prev_off_track[env_id] == 0
    progress_mask = off_track
    if reward.lee_wall_mode != 0:
        progress_mask = wall_contact
    progress_ds = delta_s
    if progress_mask:
        progress_ds = 0.0
    ...
    if reward.lee_wall_mode != 0:
        if wall_contact:
            out.wall_contact = -reward.wall_contact_coefficient * wp.sqrt(
                speed_squared
            )
    else:
        if off_track:
            out.oob = -reward.oob * speed_squared
        if first_boundary_contact:
            out.boundary_contact = -reward.boundary_contact
    ...
    if termination.full_car_out != 0:
        oob_done = full_out_left or full_out_right
    else:
        oob_done = wall_contact
    if oob_done and reward.lee_wall_mode == 0:
        out.oob_impact = (
            -reward.oob_impact
            * reward.terminal_oob_skip_seconds
            * 12.96
            * speed_squared
        )
    stopped = (
