...
case "$VERDICT" in
  win)  "$REPO/tools/progab_followup.sh" win ;;
  null|harm) "$REPO/tools/progab_followup.sh" "$VERDICT" ;;
  void) "$REPO/tools/progab_followup.sh" void 4.5 ;;
  *) log "unknown verdict"; exit 1;;
esac

log "follow-up launched for verdict=$VERDICT"
